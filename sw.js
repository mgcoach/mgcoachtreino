// TRAIN.FUEL — Service Worker v2.10.0
// Migração base64 → arquivos: pré-cache AGRESSIVO dos 36 GIFs no primeiro load
// pra manter UX idêntica ao base64 (zero buffering em rede ruim).

const CACHE_NAME = 'trainfuel-v2-10-1';
...
// 45 GIFs de exercícios — pré-cacheados em background no primeiro load
const GIFS = Array.from({ length: 45 }, (_, i) => {
...
self.addEventListener('install', (event) => {
  console.log('[SW v2.10.0] install — cacheando shell');
  event.waitUntil(
...
self.addEventListener('activate', (event) => {
  console.log('[SW v2.10.0] activate — limpando caches antigos');
  event.waitUntil(
...
async function precacheGifsInBackground() {
  try {
    ...
    console.log(`[SW v2.10.0] iniciando pré-cache de ${GIFS.length} GIFs em background`);
    ...
    console.log(`[SW v2.10.0] pré-cache concluído: ${done}/${GIFS.length} GIFs`);
  } catch (err) {
