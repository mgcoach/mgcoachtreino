// TRAIN.FUEL — Service Worker v2.10.2
...
const CACHE_NAME = 'trainfuel-v2-10-2';
...
self.addEventListener('install', (event) => {
  console.log('[SW v2.10.2] install — cacheando shell');
  event.waitUntil(
...
self.addEventListener('activate', (event) => {
  console.log('[SW v2.10.2] activate — limpando caches antigos');
  event.waitUntil(
...
async function precacheGifsInBackground() {
  try {
    ...
    console.log(`[SW v2.10.2] iniciando pré-cache de ${GIFS.length} GIFs em background`);
    ...
    console.log(`[SW v2.10.2] pré-cache concluído: ${done}/${GIFS.length} GIFs`);
  } catch (err) {
